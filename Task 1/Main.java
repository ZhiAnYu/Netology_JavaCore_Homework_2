...
public class Main {
    public static void main(String[] args) {
        ...
        for (File file : directory) {
            logger.append(file.mkdir() ? file.getName() + " directory created \n" : file.getName() + " directory not created \n");
            System.out.println(file.getName()+" on done");
        }

        for (File file : files) {
            try {
                logger.append(file.createNewFile() ? file.getName() + " file created \n" : file.getName() + " file not created \n");
            } catch (IOException e) {
