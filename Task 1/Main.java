...
public class Main {
    public static void main(String[] args) {
        ...
        for (File file : directory) {
            logger.append(LocalDateTime.now() + " " + isDirectoryCreated(file));
            System.out.println(file.getName() + " on done");
        }

        for (File file : files) {
            logger.append(LocalDateTime.now() + " " + isFileCreated(file));
            System.out.println(file.getName() + " on done");
        }
    ...
    static String isFileCreated(File file) {
        try {
            return file.createNewFile() ? file.getAbsolutePath() + " file created \n"
    ...
    static String isDirectoryCreated(File file) {
        return file.mkdir() ? file.getAbsolutePath() + " directory created \n"
